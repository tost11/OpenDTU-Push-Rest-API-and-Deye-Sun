...
private:
    ...
    void publishField(std::shared_ptr<BaseInverterClass> inv, ChannelType_t type, ChannelNum_t channel, byteAssign_fieldDeviceClass_t fieldType, bool clear = false);
    ...
    void publishInverterNumber(std::shared_ptr<BaseInverterClass> inv, const char* caption, const char* icon, const char* category, const char* commandTopic, const char* stateTopic, const char* unitOfMeasure, int16_t min = 1, int16_t max = 100);
    ...
    void createDeviceInfo(JsonObject& object, std::shared_ptr<BaseInverterClass> inv);
